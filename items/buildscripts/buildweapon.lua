...
--法杖主要技能名的元素部分
 ...
 then
   replacePatternInData(config, nil, "<elementalName>", "火焰")
 end
 if (elementalType=="ice")
 then
   replacePatternInData(config, nil, "<elementalName>", "冰霜")
 end
 if (elementalType=="electric")
 then
   replacePatternInData(config, nil, "<elementalName>", "電弧")
 end
 if (elementalType=="poison")
 then
   replacePatternInData(config, nil, "<elementalName>", "劇毒")
 end
  ...
  if config.twoHanded then
    config.tooltipFields.handednessLabel = "雙手"
  else
    config.tooltipFields.handednessLabel = "單手"
  end
